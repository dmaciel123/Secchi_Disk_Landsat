...
DATA = fread('Data/tm_acolite_Maciel_v3.csv') %>% na.omit()
DATA = index_calc_bind(df = DATA, blue = DATA$blue, green = DATA$green, red = DATA$red)


qaa_RGB = read.xlsx('Data/QAA_RGB/landsat_TM_Matchups_qaa_rgb.xlsx')

RF = readRDS('Outputs/ML_Models/random_forest_tm.R')
SVM = readRDS('Outputs/ML_Models/SVM_tm.R')
XGB = readRDS("Outputs/ML_Models/XGB_tm.R")

FILENAME = 'Outputs/Satellite_Application/comparison_TM.jpeg'


xgb_test  = xgb.DMatrix(data = as.matrix(DATA[,c('blue', 'green', 'red', 'green_red', 'blue_green','blue_red' ,'LH')], label = valid$secchi))


DATA$RF  = predict(RF, DATA)
DATA$QAA_RGB = qaa_RGB[qaa_RGB$uid %in% DATA$uid, 'Predicted']
DATA$XGBOOTS = predict(XGB, xgb_test)
DATA$SVM = predict(SVM, DATA, na.rm = F)
